 ...
 * @package    Controller
 */
class CategorieTree extends Widget
{
    ...
    public function generate()
    {
        ...
        {
            $objPage = $this->Database->prepare("SELECT id FROM tl_shop_warengruppen WHERE pid=? ORDER BY sorting")
                                      ->execute(0);
    ...
    <li class="tl_folder_top"><div class="tl_left">'.$this->generateImage((strlen($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['icon']) ? $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['icon'] : 'pagemounts.gif')).' '.('acquistoShop - ' . $GLOBALS['TL_LANG']['MOD']['acquistoShopWarengruppen'][0]).'</div> <div class="tl_right"><label for="ctrl_'.$this->strId.'" class="tl_change_selected">'.$GLOBALS['TL_LANG']['MSC']['changeSelected'].'</label> <input type="checkbox" name="'.$this->strName.'_save" id="ctrl_'.$this->strId.'" class="tl_tree_checkbox" value="1" onclick="Backend.showTreeBody(this,\''.$this->strId.'_parent\')"></div><div style="clear:both"></div></li><li class="parent" id="'.$this->strId.'_parent"><ul>'.$tree.$strReset.'
  </ul></li></ul>';
    }
    ...
    protected function renderPagetree($id, $intMargin, $protectedPage=false)
    {
        ...
        $objPage = $this->Database->prepare("SELECT id, type, published, title FROM tl_shop_warengruppen WHERE id=?")
                                  ->limit(1)
                                  ->execute($id);
        ...
        $objNodes = $this->Database->prepare("SELECT id FROM tl_shop_warengruppen WHERE pid=? ORDER BY sorting")
                                   ->execute($id);
        ...
        if (!empty($childs))
        {
            ...
            $return .= '<a href="'.$this->addToUrl($flag.'tg='.$id).'" title="'.specialchars($alt).'" onclick="Backend.getScrollOffset();return acquistoShop.toggleCategorietree(this,\''.$xtnode.'_'.$id.'\',\''.$this->strField.'\',\''.$this->strName.'\','.$level.')">'.$this->generateImage($img, '', 'style="margin-right:2px"').'</a>';
        }

        $sub = 0;
//        $image = $objPage->type.'.gif';
        $image = '/system/modules/acquistoShop/html/categorieTree/' . $objPage->type.'.png';
    ...
    protected function getPathNodes()
    {
        ...
        foreach ($this->varValue as $id)
        {
            do
            {
                $objPage = $this->Database->prepare("SELECT pid FROM tl_shop_warengruppen WHERE id=?")
                                          ->limit(1)
                                          ->execute($id);
