 ...
 * @version SofortLib 1.3.0  $Id: class.invoice.inc.php 1123 2011-07-07 09:28:22Z dehn $
...
class PnagInvoice extends PnagAbstractDocument {
	 ...
	 * @param string $apiUrl
	 */
	function PnagInvoice($apiKey, $transactionId = '', $apiUrl = 'https://api.sofort.com/api/xml/') {
		$this->transactionId = $transactionId;
		$this->apiKey = $apiKey;
		$this->apiUrl = $apiUrl;

		$this->multipay = new SofortLib_Multipay($this->apiKey, $apiUrl);
		if($transactionId != '') {
	...
	function _setupTransactionData() {
		...
		$response = $obj->setTransaction($this->transactionId)->sendRequest();
		...
		if (isset ($transactionData['items'] ) ) {
			$itemArray = $transactionData['items'];
		} else {
			$itemArray = array();
		}
...
/* ########################## WRAPPER FUNCTIONS MULTIPAY ########################## */
	...
	function getTransactionInfo() {
		if(is_a($this->transactionData, 'SofortLib')) {
			$this->transactionData->setTransaction($this->transactionId)->sendRequest();
...
/* ########################## WRAPPER FUNCTIONS MULTIPAY ########################## */
	...
	*/
	...
	function checkPnagShopArticleDifferences() {
		...
		foreach ($shopArticles as $shopArticle){
			...
			foreach ($pnagArticles as &$pnagArticle){
				...
				if ($pnagArticle->item_id == $itemId) {
					...
					$pnagArticle->setDifferenceFound (1);
				}
			}
			if (!$articleFound){
		...
		foreach ($notFoundInResponse as &$value){
			$value ['syncAction'] = $this->_getDetailedDifferenceProblem ($value ['foundInResponse'], $value ['foundInShop'] );
			$differences[] = $value;
		}
		foreach ($notFoundInShopsystem as &$value){
			$value ['syncAction'] = $this->_getDetailedDifferenceProblem ($value ['foundInResponse'], $value ['foundInShop'] );
			$differences[] = $value;
		}
		return $differences;
